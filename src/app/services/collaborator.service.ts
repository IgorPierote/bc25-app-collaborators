...
export class CollaboratorService {
  ...
  public createCollaborator(collaborator: Collaborator): Observable<any> {
    ...
    return from(promise).pipe(
      catchError(error => {
        this.notification.showMenssage("Erro ao cadastrar.");
        console.error(error);
        return EMPTY;
      })
    );
  ...
  public findAll(): Observable<any> {
    ...
    return from(promise).pipe(
      ...
      catchError(error => {
        this.notification.showMenssage("Erro ao buscar dados.");
        console.error(error);
        return EMPTY;
      })
    );
  ...
  public findById(id: string): Observable<any> {
    ...
    return from(promise).pipe(
      ...
      catchError(error => {
        this.notification.showMenssage("Erro ao buscar pelo ID");
        console.error(error);
        return EMPTY;
      })
    );
  ...
  public deleteCollaborator(id: string) {
    ...
    return from(promise).pipe(
      catchError(error => {
        this.notification.showMenssage("Erro ao excluir.");
        console.error(error);
        return EMPTY;
      })
    );
  ...
  public updateCollaborator(collaborator: Collaborator) {
    ...
    return from(promise).pipe(
      catchError(error => {
        this.notification.showMenssage("Erro ao atualizard.");
        console.error(error);
        return EMPTY;
      })
    );
